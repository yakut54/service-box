...
return new class extends Migration
{
    ...
    {
        // Update create_shop_schema to include priority in discounts for new shops
        DB::statement(<<<'SQL'
            ...
            BEGIN
                ...
                EXECUTE format('CREATE INDEX ON %I.discounts(priority DESC)', p_schema_name);
...
SQL
        ...
        // Add priority column to all existing tenant schemas
        $schemas = DB::select("
            SELECT schema_name
        ...
        foreach ($schemas as $schema) {
            ...
            DB::statement("CREATE INDEX IF NOT EXISTS discounts_priority_idx ON \"{$s}\".discounts(priority DESC)");
        }
    }
    ...
    public function down(): void
    {
        ...
        foreach ($schemas as $schema) {
            $s = $schema->schema_name;
            DB::statement("DROP INDEX IF EXISTS \"{$s}\".discounts_priority_idx");
            DB::statement("ALTER TABLE \"{$s}\".discounts DROP COLUMN IF EXISTS priority");
        }
    }
};

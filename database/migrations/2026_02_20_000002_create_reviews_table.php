...
return new class extends Migration
{
    ...
    {
        // 1. Update the schema function so new shops get the reviews table
        DB::statement(<<<'SQL'
            ...
            BEGIN
                ...
                -- reviews (after products, customers, orders)
                EXECUTE format($sql$
...
SQL
        ...
        foreach ($schemas as $schema) {
            ...
            DB::statement("CREATE INDEX IF NOT EXISTS reviews_product_published_idx ON \"{$s}\".reviews(product_id, is_published)");
            DB::statement("CREATE INDEX IF NOT EXISTS reviews_published_idx ON \"{$s}\".reviews(is_published)");
        }
    }
    ...
    public function down(): void
    {
        ...
        foreach ($schemas as $schema) {
            $s = $schema->schema_name;
            DB::statement("DROP TABLE IF EXISTS \"{$s}\".reviews");
        }
    }
};

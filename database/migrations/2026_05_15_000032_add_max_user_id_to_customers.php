...
return new class extends Migration
{
    ...
    {
        // Add column to all existing shop schemas
        $schemas = DB::select("
        ...
        foreach ($schemas as $row) {
            DB::statement("
                ALTER TABLE \"{$row->schema_name}\".customers
                ADD COLUMN IF NOT EXISTS max_user_id BIGINT DEFAULT NULL
            ");
        }

        // Update create_shop_schema function so new shops also get the column
        DB::statement(<<<'SQL'
...
SQL
    ...
    public function down(): void
    {
        ...
        foreach ($schemas as $row) {
            DB::statement("
                ALTER TABLE \"{$row->schema_name}\".customers
                DROP COLUMN IF EXISTS max_user_id
            ");
        }
    }
};

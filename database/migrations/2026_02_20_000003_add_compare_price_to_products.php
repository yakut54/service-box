...
return new class extends Migration
{
    ...
    {
        // 1. Update create_shop_schema() — добавляем compare_price в products
        DB::statement(<<<'SQL'
...
SQL
        ...
        // 2. Добавить compare_price в существующие схемы
        $schemas = DB::select("
            SELECT schema_name
    ...
    public function down(): void
    {
        ...
        foreach ($schemas as $schema) {
            $s = $schema->schema_name;
            DB::statement("ALTER TABLE \"{$s}\".products DROP COLUMN IF EXISTS compare_price");
        }
    }
};

...
return new class extends Migration
{
    ...
    {
        // Add column to all existing tenant schemas
        $schemas = DB::select("
        ...
        foreach ($schemas as $row) {
            ...
            $exists = DB::selectOne("
                ...
                WHERE table_schema = ? AND table_name = 'bookings' AND column_name = 'rating_sent'
            ", [$s]);

            if (!$exists) {
                DB::statement("ALTER TABLE {$s}.bookings ADD COLUMN rating_sent BOOLEAN NOT NULL DEFAULT FALSE");
            }
        }

        // Update create_shop_schema so new shops also get rating_sent
        DB::statement(<<<'SQL'

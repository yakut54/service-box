...
/**
 * Комиссия платформы (20%, см. PLAN.md) — снапшот суммы на заказ, чтобы
 * при будущем изменении ставки старые заказы не пересчитывались задним
 * числом. payout_status — для сверки выплат шоперу через сплит ЮKassa,
 * когда её подключим (см. PLAN.md → блокер на стороне ЮKassa).
 */
...
SQL);
        ...
        // Существующие магазины — точечный ALTER, без пересоздания схемы.
        // Без CHECK-constraint здесь (в отличие от новых схем выше) — чтобы
        // не городить условное добавление constraint по схемам; допустимые
        // значения и так контролируются на уровне приложения.
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            DB::statement('ALTER TABLE "'.$schema.'".orders ADD COLUMN IF NOT EXISTS commission_amount INTEGER NOT NULL DEFAULT 0');
            DB::statement('ALTER TABLE "'.$schema.'".orders ADD COLUMN IF NOT EXISTS payout_status TEXT NOT NULL DEFAULT \'pending\'');
        }
    }
    ...
    {
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            DB::statement('ALTER TABLE "'.$schema.'".orders DROP COLUMN IF EXISTS commission_amount');
            DB::statement('ALTER TABLE "'.$schema.'".orders DROP COLUMN IF EXISTS payout_status');
        }
    }
};

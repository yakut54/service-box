...
return new class extends Migration
{
    ...
    {
        // 1. Update create_shop_schema() — добавляем categories + category_id в products
        DB::statement(<<<'SQL'
            ...
            BEGIN
                ...
                -- categories (до products — products ссылается на categories)
                EXECUTE format($sql$
...
SQL
        ...
        // 2. Применить изменения ко всем существующим схемам магазинов
        $schemas = DB::select("
            SELECT schema_name
        ...
        foreach ($schemas as $schema) {
            ...
            // Создать таблицу categories если не существует
            DB::statement("
            ...
            DB::statement("CREATE UNIQUE INDEX IF NOT EXISTS categories_slug_unique ON \"{$s}\".categories(slug) WHERE deleted_at IS NULL");
            DB::statement("CREATE INDEX IF NOT EXISTS categories_parent_id_idx ON \"{$s}\".categories(parent_id)");
            DB::statement("CREATE INDEX IF NOT EXISTS categories_is_visible_idx ON \"{$s}\".categories(is_visible)");

            // Добавить category_id в products
            DB::statement("ALTER TABLE \"{$s}\".products ADD COLUMN IF NOT EXISTS category_id UUID REFERENCES \"{$s}\".categories(id) ON DELETE SET NULL");
            DB::statement("CREATE INDEX IF NOT EXISTS products_category_id_idx ON \"{$s}\".products(category_id)");

            // Удалить старый текстовый столбец category
            DB::statement("ALTER TABLE \"{$s}\".products DROP COLUMN IF EXISTS category");
        }
    }
    ...
    public function down(): void
    {
        ...
        foreach ($schemas as $schema) {
            ...
            DB::statement("ALTER TABLE \"{$s}\".products DROP COLUMN IF EXISTS category_id");
            DB::statement("ALTER TABLE \"{$s}\".products ADD COLUMN IF NOT EXISTS category TEXT");
            DB::statement("DROP TABLE IF EXISTS \"{$s}\".categories CASCADE");
        }
    }
};

...
/**
 * Email не должен быть обязательным при оформлении заказа — лишнее трение
 * в чекауте, телефон уже даёт identity (Customer::findOrCreateByPhone).
 * Веб-виджет и так трактует email как опциональный на фронте (см.
 * widget/src/components/Checkout.vue), просто бэкенд был не готов это
 * принять. bookings.customer_email уже был nullable — сюда просто
 * подтягиваем orders к тому же стандарту.
 */
...
SQL);
        ...
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            DB::statement('ALTER TABLE "'.$schema.'".orders ALTER COLUMN customer_email DROP NOT NULL');
        }
    }
    ...
    {
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            DB::statement('UPDATE "'.$schema.'".orders SET customer_email = \'\' WHERE customer_email IS NULL');
            DB::statement('ALTER TABLE "'.$schema.'".orders ALTER COLUMN customer_email SET NOT NULL');
        }
    }
};

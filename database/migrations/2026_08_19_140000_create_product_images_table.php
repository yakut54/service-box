...
return new class extends Migration
{
    ...
    {
        // 1) Новые магазины — обновляем create_shop_schema(), чтобы product_images
        // создавалась сразу при регистрации.
        DB::unprepared(<<<'SQL'
...
SQL);
        ...
        // 2) Существующие магазины — добавляем таблицу без пересоздания схемы
        ...
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            ...
            ');
            DB::statement('CREATE INDEX IF NOT EXISTS product_images_product_id_idx ON "'.$schema.'".product_images(product_id)');
        }
    }
    ...
    {
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            DB::statement('DROP TABLE IF EXISTS "'.$schema.'".product_images');
        }
    }
};

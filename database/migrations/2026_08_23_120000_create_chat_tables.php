...
return new class extends Migration
{
    ...
    {
        // 1) Новые магазины — обновляем create_shop_schema(), чтобы chat_threads/
        // chat_messages создавались сразу при регистрации.
        DB::unprepared(<<<'SQL'
...
SQL);
        ...
        // 2) Существующие магазины — добавляем таблицы без пересоздания схемы
        // (create_shop_schema делает DROP SCHEMA CASCADE, это уничтожило бы
        // все текущие данные — для уже живых шоперов нужен точечный ALTER).
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            ...
            ');
            DB::statement('CREATE UNIQUE INDEX IF NOT EXISTS chat_threads_customer_id_idx ON "'.$schema.'".chat_threads(customer_id)');
            DB::statement('CREATE INDEX IF NOT EXISTS chat_threads_last_message_at_idx ON "'.$schema.'".chat_threads(last_message_at DESC)');
            ...
            ');
            DB::statement('CREATE INDEX IF NOT EXISTS chat_messages_thread_id_created_at_idx ON "'.$schema.'".chat_messages(thread_id, created_at)');
            DB::statement('CREATE UNIQUE INDEX IF NOT EXISTS chat_messages_thread_client_id_idx ON "'.$schema.'".chat_messages(thread_id, client_message_id) WHERE client_message_id IS NOT NULL');
        }
    }
    ...
    {
        foreach (DB::table('shops')->pluck('schema_name') as $schema) {
            DB::statement('DROP TABLE IF EXISTS "'.$schema.'".chat_messages');
            DB::statement('DROP TABLE IF EXISTS "'.$schema.'".chat_threads');
        }
    }
};

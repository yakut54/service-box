...
return new class extends Migration
{
    ...
    {
        // 1. Delivery settings column on global shops table
        Schema::table('shops', function (Blueprint $table) {
            $table->jsonb('delivery_settings')->nullable()->after('widget_config');
        });

        // 2. Delivery columns on all existing tenant orders tables
        $schemas = DB::select("
        ...
        // 3. Update create_shop_schema — fully synced with all migrations to date
        DB::statement(<<<'SQL'
            ...
            BEGIN
                ...
                EXECUTE format('CREATE INDEX ON %I.master_services(product_id)', p_schema_name);
                ...
                EXECUTE format('CREATE INDEX ON %I.telegram_link_tokens(customer_phone)', p_schema_name);
...
SQL
    ...
    {
        Schema::table('shops', function (Blueprint $table) {
            $table->dropColumn('delivery_settings');
        });

...
const inputClass = computed(() => {
  const base = props.withGenerate ? 'input pr-20' : 'input pr-10'
  if (props.status === 'error')   return `${base} !border-red-500 focus:!ring-red-500`
  if (props.status === 'success') return `${base} !border-green-500 focus:!ring-green-500`
  return base
})

...
// Живая проверка на каждую загрузку — не хранится и не "отмечается прочитанным",
// чтобы не могло получиться "почта не настроена, но горит зелёным" (было так до
// 2026-08-30 — суточная проверка врала в промежутках между запусками).
...
async function load() {
  ...
  error.value = ''
  try {
    const resp = await api.getMailFailures()
    failures.value = resp.data
    mailConfigured.value = resp.mail_configured
  } catch (e: unknown) {
...
<template>
  <div class="card">
    ...
    <template v-else>
      <!-- Живой статус — не гасится "отметить прочитанным", видна пока реально не настроят -->
      <div v-if="!mailConfigured" class="flex items-center gap-3 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg mb-3">
        <svg class="w-6 h-6 text-red-600 dark:text-red-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
        <div>
          <div class="font-medium text-red-800 dark:text-red-300">Почта не настроена</div>
          <div class="text-sm text-red-600 dark:text-red-400">Письма никому не отправляются — сервер работает в тестовом режиме</div>
        </div>
      </div>

      <div v-if="mailConfigured && failures.length === 0" class="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
      ...
      <ul v-if="failures.length > 0" class="space-y-2">
        <li
          v-for="f in failures"
          :key="f.id"

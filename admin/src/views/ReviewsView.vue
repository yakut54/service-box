...
// ── Delete ────────────────────────────────────────────────────
function confirmDelete(r: Review) { deleteTarget.value = r }
...
<template>
  <div class="space-y-6">
    ...
    <div v-else-if="reviews.length === 0" class="card text-center py-16">
      <div class="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-4">
        <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
        </svg>
      </div>
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">Нет отзывов</h3>
      <p class="text-gray-500 dark:text-gray-400">Клиенты ещё не оставили отзывы</p>
    </div>
      ...
      >
        <div class="flex items-start gap-4">
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            <!-- Delete -->
            <button @click="confirmDelete(r)" class="text-gray-400 hover:text-red-500 transition-colors p-1.5">
    ...
    <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Удалить отзыв?</h2>

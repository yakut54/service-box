  ...
  <div class="max-w-3xl">
    ...
    <template v-else>
      <div class="flex items-start justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Заказ #{{ order.id.slice(0, 8) }}</h1>
          <p class="text-gray-500 dark:text-gray-400 mt-1">{{ formatDate(order.created_at) }}</p>
        </div>
      ...
      <div v-if="order.status !== 'completed' && order.status !== 'cancelled'" class="card mb-6">
        <h3 class="font-medium text-gray-900 dark:text-white mb-3">Изменить статус</h3>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="card">
          <h3 class="font-semibold text-gray-900 dark:text-white mb-4">Клиент</h3>
          <div class="space-y-3">
            <div><div class="text-sm text-gray-500 dark:text-gray-400">Имя</div><div class="font-medium">{{ order.customer_name }}</div></div>
            <div><div class="text-sm text-gray-500 dark:text-gray-400">Телефон</div><a :href="`tel:${order.customer_phone}`" class="font-medium text-primary-600">{{ order.customer_phone }}</a></div>
            <div v-if="order.customer_email"><div class="text-sm text-gray-500 dark:text-gray-400">Email</div><a :href="`mailto:${order.customer_email}`" class="font-medium text-primary-600">{{ order.customer_email }}</a></div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="space-y-3">
            <div v-if="order.paid_at"><div class="text-sm text-gray-500 dark:text-gray-400">Оплачен</div><div>{{ formatDate(order.paid_at) }}</div></div>
            <div><div class="text-sm text-gray-500 dark:text-gray-400">Сумма</div><div class="text-xl font-bold text-green-600">{{ formatPrice(order.total_price) }}</div></div>
          </div>
        </div>

        <div v-if="order.shipping_address" class="card">
          <h3 class="font-semibold text-gray-900 dark:text-white mb-4">Доставка</h3>
        ...
        <div v-if="order.notes" class="card">
          <h3 class="font-semibold text-gray-900 dark:text-white mb-4">Примечание</h3>
      ...
      <div class="card mt-6">
        ...
        <div class="divide-y divide-gray-100 dark:divide-gray-700">
          <div v-for="item in order.items" :key="item.id" class="py-3 flex items-center justify-between">
            <div>
              ...
              <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatPrice(item.price) }} x {{ item.quantity }}</div>
            </div>
            <div class="font-semibold dark:text-gray-200">{{ formatPrice(item.price * item.quantity) }}</div>
          </div>
        </div>
        <div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4 flex justify-between">
          ...
          <span class="text-xl font-bold text-gray-900 dark:text-white">{{ formatPrice(order.total_price) }}</span>
        </div>
      </div>
    </template>
  </div>

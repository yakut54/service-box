...
// ── Image upload (same pattern as ProductEditView) ─────────────────────────
...
// ── Parent options for modal (only top-level, excluding self and its children) ─
...
<template>
  <div class="space-y-6">
    ...
    <!-- Empty -->
    <div v-else-if="categoriesStore.categories.length === 0" class="card text-center py-16">
      <div class="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-4">
        <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16" />
        </svg>
      </div>
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">Нет категорий</h3>
      <p class="text-gray-500 dark:text-gray-400 mb-6">Создайте первую категорию для товаров</p>
        ...
        <div class="card p-0">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              ...
              <button
                ...
                :class="[
                  ...
                  cat.is_visible ? 'bg-primary-600' : 'bg-gray-300 dark:bg-gray-600'
                ]"
                :title="cat.is_visible ? 'Скрыть' : 'Показать'"
              >
              ...
              <button @click="openEdit(cat)" class="btn-ghost btn-sm text-xs px-2">
                Изменить
              </button>
          ...
          <div v-if="cat.children && cat.children.length > 0" class="border-t border-gray-100 dark:border-gray-700">
            ...
            >
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                <button
                  @click="toggleVisible(child)"
                  :disabled="toggling === child.id"
                  :class="[
                    'relative w-9 h-5 rounded-full transition-colors',
                    child.is_visible ? 'bg-primary-600' : 'bg-gray-300 dark:bg-gray-600'
                  ]"
                >
                ...
                <button @click="openEdit(child)" class="btn-ghost btn-sm text-xs px-2">
                  Изменить
                </button>
        ...
        <div class="p-4 space-y-4">
            ...
            <div v-if="form.image_url && !imageError" class="flex items-start gap-3">
              <div class="relative flex-shrink-0">
                <img
                  ...
                  class="h-20 w-20 object-cover rounded-xl border border-gray-200 dark:border-gray-700"
                  alt="Превью"
                />
          ...
          <div class="flex items-center justify-between py-2">
            ...
            <button
              ...
              :class="['relative w-11 h-6 rounded-full transition-colors', form.is_visible ? 'bg-primary-600' : 'bg-gray-300 dark:bg-gray-600']"
            >
    ...
    <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 space-y-5">
        ...
        <div class="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
          ...
          <button
            @click="hideCategory(deleteTarget); deleteTarget = null"
            class="btn-secondary btn-sm text-amber-700 dark:text-amber-400 border-amber-300 dark:border-amber-700 hover:bg-amber-100 dark:hover:bg-amber-900/30"
          >
            Скрыть с витрины
          </button>
        </div>
        ...
        <div class="border-t border-gray-100 dark:border-gray-700 pt-4 space-y-4">
          ...
          <!-- Подтверждение именем -->
          <div>
            <label class="label text-sm">
              Введите <strong>{{ deleteTarget.name }}</strong> для подтверждения
            </label>
            <input
          ...
          <div class="flex gap-3">
            ...
            <button
              ...
              :disabled="deleting || !deleteConfirmValid || (deleteAction === 'move' && !deleteMoveTarget)"
            >

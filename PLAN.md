...
└── database/         — schema/pgsql-schema.sql (squash) + 7 актуальных миграций
```
...
57 миграций сжаты в:
- `database/schema/pgsql-schema.sql` — полный снимок public-схемы (все таблицы + функция `create_shop_schema`)
- 7 PHP-миграций с данными: план_pricing cleanup/limits, конвертация цен, фикс тестовых данных, скрытие сиротских товаров, backfill колонок, обновление функции

Для новых инсталляций: Laravel загружает SQL-дамп → запускает 7 PHP-миграций (все идемпотентны на пустой БД).

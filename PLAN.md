...
| М1.4 | `resolveShopAndRole` возвращает роль `master` | ⬜ |
...
### Этап М3 — Telegram для мастера

> Конкурентное преимущество — ни у кого нет нативно.

| # | Что | Статус |
|---|---|---|
| М3.1 | Мастер привязывает свой Telegram через deep-link (аналог клиентской привязки) | ⬜ |
| М3.2 | Уведомление мастеру: новая запись к нему | ⬜ |
| М3.3 | Уведомление мастеру: запись отменена / перенесена | ⬜ |
| М3.4 | Напоминание мастеру за 2ч до начала смены | ⬜ |
| М3.5 | Кнопки в боте: "Клиент пришёл" / "Не пришёл" | ⬜ |

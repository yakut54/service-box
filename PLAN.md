...
## 🔜 ImageUpload — UX улучшения (на паузе)

### Задача: оверлей-кнопки на изображении

Текущее состояние: кнопки «Заменить» и «Удалить» — текстовые ссылки справа от картинки. URL поле показывается даже когда картинка уже загружена.

По best practices:
1. **Оверлей-кнопки:** при наведении на изображение появляется полупрозрачный тёмный слой с двумя иконками — 📷 (заменить, по центру/снизу) и ✕ (удалить, верхний правый угол)
2. **URL-поле:** показывать только в пустом состоянии (дропзона), скрывать когда изображение загружено

Затрагивает: `admin/src/components/ImageUpload.vue` (единый переиспользуемый компонент).
Все места использования (товары, категории, мастера, логотип виджета) получат улучшение автоматически.

...
| Команда (Roles) | Владелец приглашает администраторов по email; admin-токен, plan gate Business/Pro — **⚠️ требует доработки (см. ниже)** |
...
## 🔜 Администраторы — Дорожная карта

> Цель: полноценное управление администраторами — как мастера, но для людей с доступом в панель управления.
> Исследованы: Yclients, Dikidi, AmoCRM, RetailCRM, Shopify.
> Вывод: запись создаётся СРАЗУ (не после принятия). Имя видно в списке с первого момента. Письмо уходит автоматически.
> Сайдбар: вкладка "Команда" → "Администраторы". Маршрут: /staff (без переименования).

### Этап А1 — База данных

| # | Что | Статус |
|---|---|---|
| А1.1 | Добавить колонку `invite_name varchar(255) nullable` в `public.shop_staff` | ⬜ |
| А1.2 | Применить на сервере, обновить `pgsql-schema.sql`, удалить файл миграции | ⬜ |

### Этап А2 — Бэкенд

| # | Что | Статус |
|---|---|---|
| А2.1 | `ShopStaff` model: добавить `invite_name` в `$fillable` | ⬜ |
| А2.2 | `StaffController::store` — принять `name` (required), сохранить как `invite_name` | ⬜ |
| А2.3 | `StaffController::index` — вернуть `invite_name` в ответе | ⬜ |
| А2.4 | `StaffController::update` — PUT `/api/admin/staff/{id}`, редактирование имени | ⬜ |
| А2.5 | `StaffController::resend` — POST `/api/admin/staff/{id}/resend`, перевыслать письмо с новым токеном (сбросить старый, создать новый +48ч) | ⬜ |
| А2.6 | Новые маршруты в `routes/api.php` | ⬜ |

### Этап А3 — Фронтенд: структура

| # | Что | Статус |
|---|---|---|
| А3.1 | `AppLayout.vue` sidebar: "Команда" → "Администраторы" | ⬜ |
| А3.2 | `types/index.ts`: добавить `invite_name?: string \| null` в `StaffMember` | ⬜ |
| А3.3 | `api.ts`: добавить методы `createAdmin(name, email)`, `updateAdmin(id, {name})`, `resendAdminInvite(id)` | ⬜ |

### Этап А4 — AdminsView (полный аналог MastersView)

> Карточная сетка (1 колонка мобайл / 2 на планшете / 3 на десктопе).
> Структура карточки: аватар-инициалы, имя, email, статус-бейдж, кнопки внизу.

| # | Что | Статус |
|---|---|---|
| А4.1 | Переделать `StaffView.vue` → карточная сетка с `PageHeader` и кнопкой "Добавить" | ⬜ |
| А4.2 | Карточка: аватар-инициалы (2 буквы из invite_name или user.name), имя, email | ⬜ |
| А4.3 | Статус-бейджи: `Активен` (зелёный) / `Ожидает` (жёлтый) / `Истёк` (красный) | ⬜ |
| А4.4 | Кнопки карточки: "Редактировать" (всегда) · "Переслать" (только Ожидает/Истёк) · "Удалить" | ⬜ |
| А4.5 | `UiConfirmDialog` при удалении | ⬜ |
| А4.6 | `UiEmptyState` когда нет администраторов | ⬜ |
| А4.7 | Мобайл 360px — карточки в 1 колонку, кнопки не обрезаются | ⬜ |

### Этап А5 — AdminFormModal

> По образцу `MasterFormModal.vue`. Общий паттерн — не копипаста.

| # | Что | Статус |
|---|---|---|
| А5.1 | Новый `AdminFormModal.vue`: поля Имя (required) + Email (required) | ⬜ |
| А5.2 | Режим "создание": POST → карточка появляется мгновенно, письмо уходит автоматически | ⬜ |
| А5.3 | Режим "редактирование": PUT → только Имя (email не меняется после создания) | ⬜ |
| А5.4 | Валидация: имя min:2, email формат, 409 если уже приглашён | ⬜ |
...
| М3.7 | Кнопки в боте: "Клиент пришёл" / "Не пришёл" — MAX | ⬜ в очереди |

### Этап М4 — Безопасность и polish

| # | Что | Статус |
|---|---|---|
| М4.1 | Middleware `RequireNotMaster` — блокирует мастеров на всех `/api/admin/*` кроме разрешённых | ⬜ в очереди |
| М4.2 | Страница принятия приглашения: роль (мастер/администратор), имя мастера в приветствии | ✅ |

### Этап М5 — Зарплата и выработка мастера (отложено)

> Все пятеро конкурентов (Yclients, Dikidi, Altegio, Fresha, SimplyBook) показывают мастеру его статистику.
> Технически требует: модель начислений / ставок, агрегация по завершённым записям, отдельный экран в портале мастера.

| # | Что | Статус |
|---|---|---|
| М5.1 | Экран "Моя статистика": кол-во записей, выполнено, неявки за период | ⬜ отложено |
| М5.2 | Выработка в рублях (завершённые записи × стоимость услуги) | ⬜ отложено |
| М5.3 | Настройка ставки/процента для мастера (владелец задаёт) | ⬜ отложено |

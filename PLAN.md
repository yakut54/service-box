...
| # | Что | Статус |
|---|---|---|
| 5.1.1 | Backend: поле `theme` в `widget_config` | 🔲 |
| 5.1.2 | Admin: карточки-превью тем (Светлая / Тёмная / Минимализм) | 🔲 |
| 5.1.3 | Widget: CSS-переменные переключаются по теме | 🔲 |

**Проверить после этапа:** открыть настройки → выбрать "Тёмная" → сохранить → открыть виджет → убедиться что тема применилась.

#### Этап 5.2 — Расширенные цвета + шрифт 🔲

Второй цветовой слот (фон виджета) + выбор шрифта из 4 вариантов.

| # | Что | Статус |
|---|---|---|
| 5.2.1 | Backend: поля `bg_color`, `font_family` в `widget_config` | 🔲 |
| 5.2.2 | Admin: color picker для фона (переиспользовать существующий) | 🔲 |
| 5.2.3 | Admin: dropdown шрифта — Системный / Roboto / Montserrat / Georgia (переиспользовать dropdown) | 🔲 |
| 5.2.4 | Widget: применить цвет фона и подгрузить шрифт | 🔲 |

**Проверить после этапа:** сменить фон на тёмный → виджет не сломался, текст читаем. Сменить шрифт → применился в виджете.
...
| # | Что | Статус |
|---|---|---|
| 5.3.1 | Iframe виджета внутри страницы настроек | 🔲 |
| 5.3.2 | postMessage от настроек в iframe при каждом изменении | 🔲 |
| 5.3.3 | Widget: слушает postMessage и применяет изменения налету | 🔲 |
| 5.3.4 | Кнопка "Смотреть как клиент" — полноэкранный превью | 🔲 |

**Проверить после этапа:** поменять цвет → виджет слева обновился без сохранения. Кнопка "Смотреть как клиент" открывает полный виджет.

...
> Синхронизация везде двусторонняя: Telegram ↔ MAX ↔ сайт (автополлинг).
...
| Б1.3 | Вызов обоих методов в `TelegramController::handleRatingCallback` после INSERT | 🔜 |
| Б1.4 | Вызов обоих методов в `MaxController::handleRatingCallback` после INSERT | 🔜 |
...
| Б2.3 | В `handleRatingCallback` (TG + MAX): найти `master_id` из booking → `ShopStaff` → послать уведомление | 🔜 |
...
**Граничный случай:** мастер не привязан к боту — тихо пропускаем, не ломаем flow.
...
| # | Что | Статус |
|---|---|---|
| П2.1 | `TelegramController`: handler `reschedule:{bookingId}` — найти booking, проверить клиента, отменить, уведомить | 🔜 |
| П2.2 | `MaxController`: handler `reschedule:{bookingId}:0` — аналогично | 🔜 |
| П2.3 | Клиент получает: «Запись отменена. Выберите новое время 👇» + ссылка на виджет с `?service_id=` | 🔜 |
| П2.4 | Шопер получает: «📅 Клиент перенёс запись» (не «отменил») — Telegram + MAX | 🔜 |
| П2.5 | Мастер получает: «📅 Запись перенесена клиентом» — Telegram + MAX (если привязан) | 🔜 |

**Важно:** текст для шопера/мастера «перенёс» а не «отменил» — разные психологические последствия.

**Синхронизация с сайтом:** `status = cancelled` → `BookingsView` автополлинг → запись пропадает из активных. Новая запись появится в момент подтверждения через виджет.
...
| # | Что | Статус |
|---|---|---|
| W3.1 | `TelegramController`: handler `winback:{customerId}:{shopId}` — найти клиента, отправить ему в Telegram | 🔜 |
| W3.2 | `MaxController`: handler `winback:{customerId}:{shopId}:0` — найти клиента, отправить в MAX | 🔜 |
| W3.3 | Логика: если у клиента есть оба бота — отправить в оба | 🔜 |
| W3.4 | После отправки: убрать кнопки с сообщения шопера (`removeButtons`) | 🔜 |
| W3.5 | Поставить `Cache::put("winback:{shopId}:{customerId}", true, now()->addDays(30))` | 🔜 |
...
**Синхронизация**: если клиент записывается → новая запись в DB → автополлинг → шопер/мастер получат уведомления стандартным путём.

**Коммит:** `feat(bots): handle winback button — send reminder to client in their messenger`

**Что проверить:**
1. Нажать «📨 Напомнить» в Telegram → клиент получил сообщение в Telegram (если привязан)
2. Нажать «📨 Напомнить» в MAX → клиент получил сообщение в MAX (если привязан)
3. Кнопка пропала с сообщения шопера (removeButtons)
4. Повторно нажать нельзя (кеш)
